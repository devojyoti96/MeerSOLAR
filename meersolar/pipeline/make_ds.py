...
def make_ds(msname,workdir,extension="png",target_scans=[],merge_scans=True,seperate_scans=True,cpu_frac=0.8,mem_frac=0.8):
    """
    ...
    ds_files=glob.glob(f"{workdir}/{ds_file_name}*.{extension}")
    return ds_files
    
def main():
    parser = argparse.ArgumentParser(
        description="Make dynamic spectra of solar scans",formatter_class=SmartDefaultsHelpFormatter
    )
    ...
    try:
        if args.msname != "" and os.path.exists(args.msname):
            ds_files=make_ds(args.msname,args.workdir,extension=args.extension,target_scans=args.target_scans,merge_scans=args.merge, seperate_scans=args.seperate,cpu_frac=float(args.cpu_frac),mem_frac=float(args.mem_frac))
            msg=0
        else:
