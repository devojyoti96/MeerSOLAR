...
def test_show_listobs(
    ...
    mock_open_func.return_value.__enter__.return_value.readlines.return_value = file_content.splitlines(True)

    if expect_exception:
        with pytest.raises(FileNotFoundError, match=f"Measurement Set not found: {msname}"):
            show_listobs(msname)
        mock_listobs.assert_not_called()
        mock_print.assert_not_called()
    else:
        show_listobs(msname)
        mock_listobs.assert_called_once_with(vis=msname, listfile=listfile, verbose=True)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["script.py", "mock.ms"], False, True, "mock.ms"),  # Valid input → call show_listobs
    ]
)
