...
from . import dem_inv_gsvd
from . import dem_reg_map
    ...
    if sum(np.isnan(dn)) == 0 and sum(np.isinf(dn)) == 0 and np.prod(dn) > 0:
        ...
        if np.prod(dem_norm0) == 1.0 or dem_norm0[0] == 0:
            ...
            #             2. Or if nothing selected will run reg once, and use solution as weighting (self norm appraoch)
                ...
                # run gsvd
                sva, svb, U, V, W = dem_inv_gsvd.dem_inv_gsvd(rmatrixin.T, L)
                # run reg map
                lamb = dem_reg_map.dem_reg_map(sva, svb, U, W, dn, edn, rgt, nmu)
            ...
            dem_reg_lwght = (np.convolve(dem_reg_lwght[1:-1], np.ones(5) / 5))[
                1:-1
            ] / np.max(dem_reg_lwght[:])
